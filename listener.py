# -*- coding: utf-8 -*-
"""
Created on Thu Mar 31 11:35:44 2022
...
def send_msg_all(pid, msg, clients):
    for client, client_info in clients.items():
        print(f"senfing {msg} tp {client_info}")
        with Client(address = (client_info["adress"], client_info["port"]), authkey = client_info["authkey"]) as conn:
            if not client == pid:
                conn.send((pid, msg))
            else:
